...
int main(int argc, char** argv) {
    try {
        ...
        const auto builder_driver_name = std::string("builder_driver");
        ...
        if (!latest_builder_artifact_dir.empty()) {
            const auto builder_driver_binary = std::filesystem::absolute(latest_builder_artifact_dir / builder_driver_name);
            if (!std::filesystem::exists(builder_driver_binary)) {
