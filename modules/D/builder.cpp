...
BUILDER_EXTERN void builder__export_interface(const builder_t* builder, library_type_t library_type) {
    const auto interface_relative_path = relative_path_t("D.h");
...
BUILDER_EXTERN void builder__export_libraries(const builder_t* builder, library_type_t library_type) {
    const auto lib_d_relative_path = relative_path_t("d");
    const auto lib_d = cpp_compiler_t::create_library(
        builder,
    ...
    builder->install_library(lib_d, lib_d_relative_path, library_type);
}
